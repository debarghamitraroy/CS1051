...
void server_log() {
    ...
    FILE *serverFile = fopen(serverLog, "a"); // 'a' = append, creates file if it doesn’t exist
    if (serverFile == NULL) {
...
int transfer(int sockfd, Packet pkt, struct sockaddr_in cliaddr, socklen_t addr_len, int seq, size_t bytesRead) {
    /*
     * int sockfd: Socket File Descriptor (returned by socket()). Used for sending/receiving data.
     * Packet pkt: The packet structure containing data and its sequence number.
     * struct sockaddr_in cliaddr: Client’s address information (IP + port).
     * socklen_t addr_len: Length of the client address structure.
     * int seq: Current sequence number (typically 0 or 1). Used for tracking packet order.
     * size_t bytesRead: Number of bytes read from a file or buffer to include in the packet.
    */
...
int main() {
    ...
    char *path = "/home/debargha/MTech/CS1051/SSH/Assignment-5";
    char folderpath[512];
