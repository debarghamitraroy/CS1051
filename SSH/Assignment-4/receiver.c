...
static void usage(const char *prog) {
    fprintf(stderr, "Usage: %s <listen_ip> <port> [output_dir] [loss_prob%%]\n",
            prog);
    fprintf(stderr, "Example: %s 0.0.0.0 9999 . 0\n", prog);
}
...
// safer version of make_out_path
static void make_out_path(char *dst, size_t dstlen, const char *outdir,
                          const char *fname) {
    char safe_name[FILENAME_MAXLEN + 1];
    strncpy(safe_name, fname, FILENAME_MAXLEN);
    safe_name[FILENAME_MAXLEN] = '\0'; // ensure null-termination
...
int main(int argc, char **argv) {
    ...
    for (;;) {
        ssize_t rc = recvfrom(sock, pktbuf, sizeof(pktbuf), 0,
        ...
        // Simulate ACK loss if requested
        if (!maybe_drop(loss_prob)) {
            sendto(sock, &ack, sizeof(ack), 0, (struct sockaddr *)&sender,
                   sender_len);
            printf("Sent ACK %u\n", seq);
        } else {
        ...
        // Initialize output file on first valid packet
        if (!fp) {
        ...
        if (hdr->eof) {
            ...
            break; // stop after one file transfer
        }

        // Accept only the expected sequence
        if (seq == expected_seq) {
            ...
            printf("Out-of-order/duplicate packet seq=%u (expected %u). "
                   "Ignored.\n",
                   seq, expected_seq);
        }
    }
